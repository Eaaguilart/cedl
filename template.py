...
def eval(model, val_loader, K=None, epoch=None):
    ...
    for images, target, task_ids in val_loader:
        ...
        loss = edl_log_loss(logits, target, epoch, K, args.num_epochs, torch.cuda.current_device())
...
if __name__ == "__main__":
    ...
    args.class_order = [68, 56, 78, 8, 23, 84, 90, 65, 74, 76, 
    ...
    scenario_train, args.nb_classes = build_dataset(is_train=True, apply_da=True, args=args)
    scenario_val, _ = build_dataset(is_train=False, apply_da=True, args=args)
    ...
    memory = rehearsal.RehearsalMemory(
        ...
        nb_total_classes = len(args.class_order) # CIFAR-100
    )
    teacher_model = None
    ...
    for task_id, dataset_train in enumerate(scenario_train):
        ...
        for epoch in range(args.num_epochs):
            ...
            for idx, (inputs, targets, task_ids) in enumerate(train_loader):
                ...
                loss_ce = edl_log_loss(logits, targets, epoch, (task_id*args.increment)+args.num_bases, args.num_epochs, episode=task_id, increment=args.increment, device=torch.cuda.current_device())
        ...
        acc1 = eval(model, val_loader, K = (task_id*args.increment)+args.num_bases, epoch = args.num_epochs)
        acc1s.append(acc1)
        print(f"task id = {task_id}  @Acc1 = {acc1:.5f}, acc1s = {acc1s}")
        torch.save(model.state_dict(), "model_"+str(task_id))
